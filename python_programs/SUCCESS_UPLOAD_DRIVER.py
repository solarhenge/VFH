...
root_folder_success_driver = common.get_property_value(properties_file_name,'root_folder_success_driver')
print('root_folder_success_driver')
print(root_folder_success_driver)
common.find_folder_name(root_folder_success_driver)
...
sql = "select distinct [customer],[month_end],[drivers_last_name],[drivers_first_name],[drivers_middle_name],[drivers_license],[drivers_id],[activation_date_of_driver],[deactivation_date_of_driver] from [%s].[dbo].[upload_easy_driver] order by [customer],[month_end],[drivers_id],[drivers_last_name],[drivers_first_name]" % (database_vfh) 
print("sql")
print(sql)

import os
import time
fname = root_folder_success_driver+os.path.splitext(sys.argv[0])[0]+'+'+time.strftime("%Y%m%d_%H%M%S")+'.csv'
print(fname)
#sys.exit()
